...
A system API key is also required which can be created in the Django admin area (see [API Documentation](#api-documentation)).
...
An API key is required for all endpoints. API keys may be generated in the admin area by navigating to "API keys" under "API" on the left. Admin-only and system-only API keys may be generated by ticking the "Is admin" or "Is system" box when creating a new key.
...
Get a random captcha question and id.
